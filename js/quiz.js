...
function render(){
  ...
  $('scoreText').textContent=mode==='exam'?`답변 ${answers.filter(x=>x?.choices?.length).length} · 검토 ${reviewed.size}`:`정답 ${queue.filter((x,i)=>isCorrect(x,answers[i])).length}`;
  $('progressFill').style.width=`${(index+1)/queue.length*100}%`;
  const source=q.officialSource?'Microsoft 공식':`자료 Q${q.sourceQuestion}`;
  const tools=mode==='exam'?`<div class="exam-tools"><div class="exam-tools-head"><button id="review" class="review-btn ${reviewed.has(index)?'active':''}">${reviewed.has(index)?'★ 검토 표시됨':'☆ 검토 표시'}</button><button id="toggleMap" class="map-toggle" aria-expanded="false">문제 목록 보기 <span>⌄</span></button></div><div class="question-map" id="questionMap" hidden>${queue.map((_,i)=>`<button data-q="${i}" class="${i===index?'current':''} ${answers[i]?.choices?.length?'answered':''} ${reviewed.has(i)?'marked':''}" aria-label="${i+1}번 문제">${i+1}</button>`).join('')}</div><div class="map-legend"><span><i class="legend-current"></i>현재</span><span><i class="legend-answered"></i>답변 완료</span><span><i class="legend-marked"></i>검토</span></div></div>`:'';
  $('quizArea').innerHTML=`<article class="question"><div class="qmeta"><span>${q.domain}</span><span>${q.topic}</span><span>${source}</span>${multi?'<span>복수 선택</span>':''}</div><div class="qnum">QUESTION ${String(index+1).padStart(2,'0')}</div><h2>${q.question}</h2>${multi?'<p class="multi-note">해당하는 답변을 모두 선택하세요.</p>':''}<div class="choices">${q.choices.map((c,i)=>`<button data-i="${i}" class="${a?.choices?.includes(i)?'selected':''}" ${locked?'disabled':''}><i>${String.fromCharCode(65+i)}</i><span>${c}</span></button>`).join('')}</div>${multi&&!locked&&mode!=='exam'?'<button class="confirm-answer" id="confirmAnswer">선택 확정</button>':''}${locked?feedback(q,a):''}${tools}<div class="qnav"><button class="btn ghost" id="prev" ${index===0?'disabled':''}>이전</button>${index===queue.length-1?`<button class="btn" id="submit">${mode==='exam'?'시험 종료 및 제출':'결과 보기'}</button>`:'<button class="btn" id="next">다음</button>'}</div></article>`;
  ...
  const tm=$('toggleMap');if(tm)tm.onclick=()=>{const map=$('questionMap'),open=map.hidden;map.hidden=!open;tm.setAttribute('aria-expanded',String(open));tm.innerHTML=open?'문제 목록 닫기 <span>⌃</span>':'문제 목록 보기 <span>⌄</span>'};
