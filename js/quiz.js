...
function render(){
  ...
  const tools=mode==='exam'?`<div class="exam-tools"><button id="review" class="review-btn ${reviewed.has(index)?'active':''}">${reviewed.has(index)?'★ 검토 표시됨':'☆ 검토 표시'}</button><div class="question-map">${queue.map((_,i)=>`<button data-q="${i}" class="${i===index?'current':''} ${answers[i]?.choices?.length?'answered':''} ${reviewed.has(i)?'marked':''}">${i+1}</button>`).join('')}</div></div>`:'';
